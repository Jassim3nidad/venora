...
// Parse .env.local, while allowing CI/staging environment values to override it.
...
const ACCOUNTS = [
  {
    role: "analystAdmin",
    email: env.E2E_ANALYST_ADMIN_EMAIL,
    password: env.E2E_ANALYST_ADMIN_PASSWORD,
    dbRole: "admin",
    tier: "analyst",
  },
  {
    role: "financeAdmin",
    email: env.E2E_FINANCE_ADMIN_EMAIL,
    password: env.E2E_FINANCE_ADMIN_PASSWORD,
    dbRole: "admin",
    tier: "finance_admin",
  },
  {
    role: "superAdmin",
    email: env.E2E_SUPERADMIN_EMAIL,
  ...
  {
    role: "venue",
    email: env.E2E_VENUE_EMAIL,
    password: env.E2E_VENUE_PASSWORD,
    dbRole: "venue_owner",
    orgName: "QA Venue Org",
    venueName: "QA Venue",
  },
  {
    role: "tenantAOwner",
    email: env.E2E_TENANT_A_OWNER_EMAIL ?? env.RLS_TENANT_A_EMAIL,
    password: env.E2E_TENANT_A_OWNER_PASSWORD ?? env.RLS_TENANT_A_PASSWORD,
    dbRole: "venue_owner",
    orgName: "Tenant A Org",
    venueName: "Tenant A Venue",
  },
  {
    role: "tenantBOwner",
    email: env.E2E_TENANT_B_OWNER_EMAIL ?? env.RLS_TENANT_B_EMAIL,
    password: env.E2E_TENANT_B_OWNER_PASSWORD ?? env.RLS_TENANT_B_PASSWORD,
    dbRole: "venue_owner",
    orgName: "Tenant B Org",
    venueName: "Tenant B Venue",
  },
  {
    role: "coordinator",
    email: env.E2E_COORDINATOR_EMAIL,
    password: env.E2E_COORDINATOR_PASSWORD,
    dbRole: "event_coordinator",
  },
  {
    role: "supplier",
    email: env.E2E_SUPPLIER_EMAIL,
    password: env.E2E_SUPPLIER_PASSWORD,
    dbRole: "supplier",
    businessName: "QA Supplier",
  },
  {
    role: "tenantACustomer",
    email: env.E2E_TENANT_A_CUSTOMER_EMAIL,
    password: env.E2E_TENANT_A_CUSTOMER_PASSWORD,
    dbRole: "customer",
  },
  {
    role: "tenantBCustomer",
    email: env.E2E_TENANT_B_CUSTOMER_EMAIL,
    password: env.E2E_TENANT_B_CUSTOMER_PASSWORD,
    dbRole: "customer",
  },
  {
    role: "nonMember",
    email: env.E2E_NON_MEMBER_EMAIL ?? env.RLS_NON_MEMBER_EMAIL,
    password: env.E2E_NON_MEMBER_PASSWORD ?? env.RLS_NON_MEMBER_PASSWORD,
    dbRole: null,
  },
...
async function provisionAccounts() {
  ...
  for (const acc of ACCOUNTS) {
    ...
    if (acc.dbRole) {
      ...
      await adminClient
        .from("user_roles")
        .insert({ user_id: userId, role: acc.dbRole });
      ...
      if (acc.tier) {
        ...
        await adminClient
          .from("admin_user_roles")
          .insert({ user_id: userId, tier: acc.tier, is_active: true });
  ...
  for (const role of ["venue", "tenantAOwner", "tenantBOwner"]) {
    ...
    if (orgId) {
      ...
      let { data: venues } = await adminClient
        .from("venues")
        .select("id")
        .eq("slug", slug);
  ...
  if (supplierId) {
    const { data: sps } = await adminClient
      .from("supplier_profiles")
      .select("id")
      .eq("profile_id", supplierId);
    if (!sps || sps.length === 0) {
      await adminClient.from("supplier_profiles").insert({
        profile_id: supplierId,
        business_name: "QA Supplier",
