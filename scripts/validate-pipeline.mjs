...
if (!supabaseUrl || !supabaseKey || !testUserId) {
  console.error('❌ Missing SUPABASE_URL, SERVICE_ROLE_KEY, or NOTIFICATION_TEST_USER_ID.');
  console.log('Skipping pipeline test. Provide NOTIFICATION_TEST_USER_ID to run full pipeline.');
  process.exit(1);
}
...
async function runPipeline() {
  ...
  deliveries.forEach(d => {
    ...
    if (d.provider === 'resend') hasEmail = true;
  ...
  if (!hasSms || deliveries.find(d => d.provider === 'twilio' && d.status === 'skipped')) {
  ...
  processedDeliveries.forEach(d => {
    if (d.status === 'failed') {
      console.error(`❌ Delivery for ${d.provider} failed: ${JSON.stringify(d.error_log)}`);
      dispatchPassed = false;
    } else if (d.status === 'queued') {
      console.log(`⚠️ Delivery for ${d.provider} is still queued (dispatcher might not be running locally).`);
