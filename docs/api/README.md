...
| `POST /api/bookings/{id}/payment`              | Start or resume deposit checkout                      | Session; booking customer through `start_booking_payment`                                              | Optional provider enum: `paymongo`, `maya`, `stripe`; default `paymongo`                                         | Creates/reuses `transactions`, creates hosted checkout, attaches provider session, sets payment pending                                           | Yes. Pending transaction lock, 55-minute session reuse, first-attach-wins race guard                | Provider limits only     |
...
| `POST /api/webhooks/maya`                      | Placeholder Maya receiver                             | `x-maya-signature` HMAC                                                                                | JSON parsed after signature; no schema library                                                                   | Failure events can call `fail_booking_payment`; success path deliberately throws because current reconciliation contract is not implemented       | No event claim/dedupe                                                                               | Provider delivery limits |
...
| `GET /api/debug`                               | Diagnostic inquiry lookup                             | No explicit guard                                                                                      | None                                                                                                             | Attempts `auth.admin.listUsers()` using normal server client, then returns first customer ID and inquiry data if privileged access somehow exists | Safe read                                                                                           | None                     |
...
| ----------------------- | ------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------- |
| `ai-search`             | Token optional; search is public                             | `query<=500` and/or filters for location, budget, guests, venue type/features, `per_page<=50` effective | Hybrid `search_venues`; writes `ai_search_logs`; may warm embeddings and write usage logs              | Admin AI limits when LLM parsing runs; each call logs                             |
...
| `booking-notifications` | Platform JWT is expected; invoked by DB webhook/service role | Delivery record, or `{limit}` batch request                                                             | Dispatches email/push/in-app, updates delivery attempts/statuses; SMS skipped                          | Queue status and retry RPC control repeats; callers must not expose this publicly |
...
- `/api/debug` has no explicit environment, session, role, or permission guard. Its normal Supabase client should not possess `auth.admin` privilege, but the route is still an undocumented diagnostic surface that should be removed or strictly gated before production.
- Maya success reconciliation is intentionally not wired to the hardened payment RPC; Maya is not a usable payment provider. It also lacks PayMongo-style event idempotency.
- `PAYMENT_PROVIDERS` advertises Maya and Stripe, while only PayMongo is registered when configured. Selecting an unregistered provider returns a mapped payment error.
...
- Edge Function JWT deployment settings are not pinned per function in `supabase/config.toml`; production configuration must preserve JWT verification, especially for `booking-notifications`.
