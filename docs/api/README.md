...
| `ai-assistant`          | Token optional for chat; customer bearer required for actions              | Chat: `sessionId<=100`, message<=2,000, optional conversation ID; action: request UUID + confirmation   | SSE/JSON proposal/action result; writes conversations, action requests, usage, and audit evidence      | AI/conversation limits; action request is conditionally claimed                   |
...
| `rsvp-notifications`    | Customer bearer for invitations; anon bearer + reminder secret for batches | Invitation guest UUID or bounded reminder mode                                                          | Sends Resend RSVP mail and records delivery/error timestamps                                           | Ownership RLS; conditional reminder claim prevents duplicates                     |
