...
| UX-18 | P2       | Payment integration                 | Backend enum accepts Maya/Stripe while only PayMongo is registered; customer UX must not imply inactive providers work                                          | Only operational providers are selectable/advertised                               | Derive UI choices from registered gateway capability; preserve PayMongo-only messaging     |
