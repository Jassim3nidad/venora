  ...
  if (status === "pending_approval") {
    ...
    if (canReject)
      actions.push({
        ...
        variant: "danger",
        requiresReason: true,
        reasonLabel: "Why is this venue being rejected?",
      });
    if (canReview)
      actions.push({
        key: "request_info",
  ...
  if (status === "published") {
    if (canSuspend)
      actions.push({
        ...
        variant: "danger",
        requiresReason: true,
        reasonLabel: "Why is this venue being suspended?",
      });
    if (canSuspend)
      actions.push({
        key: "unpublish",
...
export default async function AdminVenueDetailPage({ params }: Props) {
  ...
  async function submitReview(input: {
    ...
    "use server";
    return reviewVenueAction({
      id: input.id,
      action: input.action as VenueReviewAction,
      reason: input.reason,
    });
  }
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <div className="space-y-6 lg:col-span-2">
          <Panel>
            ...
            <dl className="grid grid-cols-2 gap-4 text-sm">
              <div>
                ...
                <dd className="text-[#111827]">{venue.ownerName ?? "—"}</dd>
              </div>
              <div>
                <dt className="font-bold text-[#64748b]">Organization</dt>
                <dd className="text-[#111827]">
                  {venue.organizationName ?? "—"}
                </dd>
              </div>
              <div className="col-span-2">
                <dt className="font-bold text-[#64748b]">Address</dt>
                <dd className="text-[#111827]">{venue.address}</dd>
              </div>
              <div>
                <dt className="font-bold text-[#64748b]">Capacity</dt>
                <dd className="text-[#111827]">
              ...
              {venue.description ? (
                <div className="col-span-2">
                  ...
                  <dd className="text-[#111827]">{venue.description}</dd>
                </div>
              ) : null}
            </dl>
          </Panel>
          ...
          <Panel>
            ...
            {mapLocation ? (
              ...
              <EmptyState
                ...
                description="This venue has no latitude/longitude on file."
              />
            )}
          </Panel>
          ...
          <Panel>
            <PanelHeader
              title="Photos"
              description={`${venue.images.length} uploaded — at least one is required before approval.`}
            />
          ...
          <Panel>
            ...
            <div className="flex flex-wrap gap-2">
              {venue.amenities.map((a) => (
                <StatusBadge key={a} status="active" label={a} />
              ))}
              {venue.eventTypes.map((e) => (
                <StatusBadge key={e} status="inactive" label={e} />
              ))}
          ...
          <Panel>
            <PanelHeader
              ...
              description="Immutable log of every decision made on this venue."
            />
