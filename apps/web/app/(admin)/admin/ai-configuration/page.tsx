...
import {
  getAiConfigurations,
  getAiUsageSummary,
} from "@/features/admin-ai-configuration/application/queries";
import { AiConfigurationCard } from "@/features/admin-ai-configuration/ui/AiConfigurationCard";
import {
  AI_FEATURE_LABELS,
  type AiUsageSummary,
} from "@/features/admin-ai-configuration/types/ai-configuration.types";
  ...
  const usageColumns: DataTableColumn<AiUsageSummary>[] = [
    {
      key: "feature",
      header: "Feature",
      cell: (row) => AI_FEATURE_LABELS[row.feature],
    },
    {
      key: "requests",
      header: "Requests (30d)",
      cell: (row) => String(row.requestCount),
    },
    {
      key: "failures",
      header: "Failures",
    ...
    },
    {
      key: "cost",
      header: "Est. cost",
      cell: (row) => `₱${(row.estimatedCostCents / 100).toFixed(2)}`,
    },
  ];
    ...
    >
      <Panel>
        ...
        <ul className="space-y-2 text-sm text-[#475569]">
          <li>
            • <strong>Key status:</strong> not shown here — this app can't read
            Supabase Edge Function secrets. Verify keys in the Supabase
            Dashboard under Edge Functions → Secrets.
          </li>
          <li>
            • <strong>Live enforcement:</strong> all 6 AI Edge Functions read
            this configuration on every request — a disabled feature is rejected
            before it ever reaches the provider, and rate/daily/spending limits
            are checked against real usage logs.
          </li>
          <li>
          ...
          <li>
            ...
            and server errors are retried once against the same approved model.
            Alternate provider/model fallback is disabled.
          </li>
          <li>
            • <strong>Moderation:</strong> when enabled, free-text inputs
            (assistant chat, search queries) are checked against a lightweight
            prompt-injection/safety heuristic before being sent to the provider
            — this is not a full content-moderation API integration.
          </li>
          <li>
            • <strong>Usage metrics:</strong> the table below reads from the
            real usage log, populated by every AI request (metadata only — no
            prompt content is ever stored in it).
          </li>
        </ul>
      </Panel>
          ...
          {configurations.map((config) =>
            canManage ? (
              ...
              <Panel key={config.feature}>
                <PanelHeader
                  title={AI_FEATURE_LABELS[config.feature]}
                  description={`${config.provider} · ${config.model} · ${config.enabled ? "Enabled" : "Disabled"}`}
                />
              </Panel>
            ),
          )}
      ...
      <Panel>
        <PanelHeader
          ...
          description="Request counts, token usage, and estimated cost — no prompt content is logged."
        />
        {usage.length > 0 ? (
          <DataTable
            rows={usage}
            columns={usageColumns}
            keyFn={(row) => row.feature}
          />
        ) : (
          <EmptyState
            icon="analytics"
