...
function actionsForStatus(
  ...
  if (status === "pending") {
    if (canApprove)
      actions.push({ key: "approve", label: "Accredit", variant: "primary" });
    if (canReject)
      actions.push({
        key: "reject",
        label: "Reject",
        variant: "danger",
        requiresReason: true,
        reasonLabel: "Why is this supplier being rejected?",
      });
    if (canReview)
      actions.push({
        key: "request_info",
        ...
        variant: "secondary",
        requiresReason: true,
        reasonLabel: "What additional information is needed?",
      });
  }
  if (status === "accredited" && canSuspend) {
    actions.push({
      ...
      variant: "danger",
      requiresReason: true,
      reasonLabel: "Why is this supplier being suspended?",
    });
  }
  if ((status === "suspended" || status === "rejected") && canSuspend) {
    actions.push({
      key: "restore",
      label: status === "suspended" ? "Restore" : "Reopen for review",
      variant: "primary",
    });
  }
  if (canReview) {
...
export default async function AdminSupplierDetailPage({ params }: Props) {
  ...
  async function submitReview(input: {
    ...
    "use server";
    return reviewSupplierAction({
      id: input.id,
      action: input.action as SupplierReviewAction,
      reason: input.reason,
    });
  }
        ...
        <div className="space-y-6 lg:col-span-2">
          <Panel>
            ...
            <dl className="grid grid-cols-2 gap-4 text-sm">
              <div>
                <dt className="font-bold text-[#64748b]">Owner</dt>
                <dd className="text-[#111827]">{supplier.ownerName ?? "—"}</dd>
              </div>
              <div>
                <dt className="font-bold text-[#64748b]">Category</dt>
                <dd className="text-[#111827]">
                  {supplier.categoryName ?? (
                    <span className="text-red-700">
                      Missing — required before accreditation
                    </span>
                  )}
                </dd>
              </div>
              <div>
                <dt className="font-bold text-[#64748b]">Contact email</dt>
                <dd className="text-[#111827]">
                  {supplier.contactEmail || "—"}
                </dd>
              </div>
              <div>
                <dt className="font-bold text-[#64748b]">Contact phone</dt>
                <dd className="text-[#111827]">
                  {supplier.contactPhone || "—"}
                </dd>
              </div>
              <div className="col-span-2">
                <dt className="font-bold text-[#64748b]">Service areas</dt>
                <dd className="text-[#111827]">
                  {supplier.serviceAreas.length > 0 ? (
                    supplier.serviceAreas.join(", ")
                  ) : (
                    <span className="text-red-700">
                      None set — required before accreditation
                    </span>
                  )}
                </dd>
              </div>
              {supplier.coverageRadiusKm ? (
                <div>
                  <dt className="font-bold text-[#64748b]">Coverage radius</dt>
                  <dd className="text-[#111827]">
              ...
              {supplier.basePrice !== null ? (
                <div>
                  <dt className="font-bold text-[#64748b]">Base price</dt>
                  <dd className="text-[#111827]">
                    ₱{supplier.basePrice.toLocaleString()} /{" "}
                    {(supplier.priceUnit ?? "").replace(/_/g, " ")}
                  </dd>
                </div>
              ) : null}
              <div className="col-span-2">
                <dt className="font-bold text-[#64748b]">
                  Cancellation policy
                </dt>
                <dd className="text-[#111827]">
                  {supplier.cancellationPolicy || "Not specified"}
                </dd>
              </div>
              {supplier.description ? (
                <div className="col-span-2">
                  <dt className="font-bold text-[#64748b]">Description</dt>
                  <dd className="text-[#111827]">{supplier.description}</dd>
                </div>
              ) : null}
            </dl>
          </Panel>
          ...
          <Panel>
            <PanelHeader
              title="Packages"
              description="Services and pricing offered."
            />
          ...
          <Panel>
            <PanelHeader
              title="Portfolio"
              description={`${supplier.portfolio.length} items`}
            />
            {supplier.portfolio.length > 0 ? (
              <div className="grid grid-cols-3 gap-3 sm:grid-cols-4">
                {supplier.portfolio.map((p) => (
                  <div
                    key={p.id}
                    className="aspect-square overflow-hidden rounded-xl border border-[#e5e7eb] bg-[#f8fafc]"
                  >
                    <img
                      src={p.imageUrl}
                      alt={p.title}
          ...
          <Panel>
            <PanelHeader
              ...
              description="Immutable log of every decision made on this supplier."
            />
