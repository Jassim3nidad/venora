...
export default async function EditVenuePage({
  ...
  const [initialDrafts, packagesResult, amenitiesResult, venueAmenitiesResult] =
    ...
    context.supabase.from("venue_amenities").select("amenity_id").eq("venue_id", id),
  ]);
  ...
  const amenities = (amenitiesResult.data ?? []) as Array<{ id: string; name: string }>;
  ...
  async function updateVenueAction(formData: FormData) {
    ...
    const cancellationPolicy = fieldValue(formData, "cancellation_policy") || null;
    ...
    if ((latitude == null) !== (longitude == null)) {
      errorRedirect(id, "Please set both latitude and longitude for the map point.");
    }
    ...
    const { error } = await actionContext.supabase
      ...
      .update({
        ...
        overnight_accommodation: booleanValue(formData, "overnight_accommodation"),
    ...
    if (deleteAmenitiesError) {
      errorRedirect(id, deleteAmenitiesError.message || "Unable to update amenities.");
    }
    ...
    if (selectedAmenityIds.length > 0) {
      ...
      if (insertAmenitiesError) {
        errorRedirect(id, insertAmenitiesError.message || "Unable to update amenities.");
      }
    }

    const packageIds = formData.getAll("package_id").map((value) => String(value));
    for (const packageId of packageIds) {
      ...
      const packagePrice = Number(fieldValue(formData, `package_${packageId}_price`));
      ...
      const shouldDeactivate = formData.get(`package_${packageId}_delete`) === "on";
      ...
      if (
        ...
        errorRedirect(id, "Package minimum guests cannot exceed maximum guests.");
      }
      if (packageMaxGuests != null && packageMaxGuests > capacityMax) {
        errorRedirect(id, "Package maximum guests cannot exceed venue capacity.");
      }
    ...
    if (newPackageName || newPackagePriceRaw) {
      ...
      const newPackageMinGuests = optionalNumber(formData, "new_package_min_guests");
      const newPackageMaxGuests = optionalNumber(formData, "new_package_max_guests");
      ...
      if (
        ...
        errorRedirect(id, "New package minimum guests cannot exceed maximum guests.");
      }
      if (newPackageMaxGuests != null && newPackageMaxGuests > capacityMax) {
        errorRedirect(id, "New package maximum guests cannot exceed venue capacity.");
      }
      ...
      const { error: newPackageError } = await actionContext.supabase
        ...
        .insert({
          ...
          price_unit: validPriceUnit(fieldValue(formData, "new_package_price_unit")),
      ...
      if (newPackageError) {
        errorRedirect(id, newPackageError.message || "Unable to create package.");
      }
    }
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_420px]">
        ...
        >
          <div className="p-5 sm:p-6">
            <div className="mb-5">
              <h2 className="text-2xl font-black tracking-[-0.02em] text-[#020617]">
                {venue.name}
              </h2>
          ...
          <form action={updateVenueAction} className="space-y-5">
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div className="flex flex-col gap-2">
                <label htmlFor="edit-venue-province" className={topLabelClass}>
                  Province
                </label>
                <input
              ...
              <div className="flex flex-col gap-2">
                <label htmlFor="edit-venue-municipality" className={topLabelClass}>
                  Municipality
                </label>
                <input
              ...
              <div className="flex flex-col gap-2">
                <label htmlFor="edit-venue-price-unit" className={topLabelClass}>
                  Base price unit
                </label>
                <select
              ...
              <div className="flex flex-col gap-2">
                <label htmlFor="edit-venue-capacity-min" className={topLabelClass}>
                  Min capacity
                </label>
                <input
              ...
              <div className="flex flex-col gap-2">
                <label htmlFor="edit-venue-capacity-max" className={topLabelClass}>
                  Max capacity
                </label>
                <input
              ...
              <div className="flex flex-col gap-2 sm:col-span-2">
                <label htmlFor="edit-venue-description" className={topLabelClass}>
                  Description
                </label>
                <textarea
            ...
            <section className="rounded-2xl border border-[#e5e7eb] bg-[#f9fafb] p-5">
              ...
              <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                {amenities.map((amenity) => (
                  <label
                    key={amenity.id}
                    className="flex items-center gap-3 rounded-xl border border-[#e5e7eb] bg-white px-3 py-3 text-sm font-semibold text-[#374151]"
                  >
                    <input
                      type="checkbox"
                      name="amenity_ids"
            ...
            <section className="rounded-2xl border border-[#e5e7eb] bg-white p-5">
              ...
              <div className="mt-5 space-y-4">
                {packages.length > 0 ? (
                  packages.map((pkg, index) => (
                    ...
                    >
                      ...
                      <div className="grid gap-4 sm:grid-cols-2">
                        <div className="flex flex-col gap-2">
                          <label className={labelClass} htmlFor={`package-${pkg.id}-name`}>
                            Package name
                          </label>
                          <input
                        ...
                        <div className="flex flex-col gap-2">
                          <label className={labelClass} htmlFor={`package-${pkg.id}-price`}>
                            Price
                          </label>
                          <input
                        ...
                        <div className="flex flex-col gap-2">
                          <label className={labelClass} htmlFor={`package-${pkg.id}-unit`}>
                            Price unit
                          </label>
                          <select
                        ...
                        <div className="grid gap-3 sm:grid-cols-2">
                          <div className="flex flex-col gap-2">
                            <label className={labelClass} htmlFor={`package-${pkg.id}-min`}>
                              Min guests
                            </label>
                            <input
                          ...
                          <div className="flex flex-col gap-2">
                            <label className={labelClass} htmlFor={`package-${pkg.id}-max`}>
                              Max guests
                            </label>
                            <input
                        ...
                        <div className="flex flex-col gap-2 sm:col-span-2">
                          <label className={labelClass} htmlFor={`package-${pkg.id}-description`}>
                            Description
                          </label>
                          <textarea
                        ...
                        <div className="flex flex-col gap-2 sm:col-span-2">
                          <label className={labelClass} htmlFor={`package-${pkg.id}-inclusions`}>
                  ...
                  <div className="rounded-2xl border border-dashed border-[#d1d5db] bg-[#f9fafb] p-4 text-sm font-semibold text-[#6b7280]">
                ...
                <div className="rounded-2xl border border-dashed border-[#bfdbfe] bg-[#eff6ff] p-4">
                  ...
                  <div className="grid gap-4 sm:grid-cols-2">
                    <div className="flex flex-col gap-2">
                      <label className={labelClass} htmlFor="new-package-name">
                        Package name
                      </label>
                      <input id="new-package-name" name="new_package_name" className={inputClass} />
                    </div>
                    <div className="flex flex-col gap-2">
                      <label className={labelClass} htmlFor="new-package-price">
                        Price
                      </label>
                      <input
                    ...
                    <div className="flex flex-col gap-2">
                      <label className={labelClass} htmlFor="new-package-unit">
                        Price unit
                      </label>
                      <select
                        id="new-package-unit"
                    ...
                    <label className="flex items-center gap-3 rounded-xl border border-[#bfdbfe] bg-white px-3 py-3 text-sm font-semibold text-[#374151]">
                      <input
                        type="checkbox"
                    ...
                    <div className="grid gap-3 sm:grid-cols-2">
                      <div className="flex flex-col gap-2">
                        <label className={labelClass} htmlFor="new-package-min">
                          Min guests
                        </label>
                        <input
                          id="new-package-min"
                      ...
                      <div className="flex flex-col gap-2">
                        <label className={labelClass} htmlFor="new-package-max">
                          Max guests
                        </label>
                        <input
                          id="new-package-max"
                    ...
                    <div className="flex flex-col gap-2 sm:col-span-2">
                      <label className={labelClass} htmlFor="new-package-description">
                        Description
                      </label>
                      <textarea
                    ...
                    <div className="flex flex-col gap-2 sm:col-span-2">
                      <label className={labelClass} htmlFor="new-package-inclusions">
                        Package inclusions, one per line
                      </label>
                      <textarea
            ...
            <section className="grid gap-5 rounded-2xl border border-[#e5e7eb] bg-white p-5 lg:grid-cols-2">
              ...
              <div className="flex flex-col gap-2 lg:col-span-2">
                <label htmlFor="edit-cancellation-policy" className={labelClass}>
              ...
              <div className="grid gap-3 sm:grid-cols-2 lg:col-span-2">
                {([
                  ...
                  <label
                    key={name}
                    className="flex items-center gap-3 rounded-xl border border-[#e5e7eb] bg-[#f9fafb] px-3 py-3 text-sm font-semibold text-[#374151]"
                  >
                    <input
                      type="checkbox"
                      name={name}
