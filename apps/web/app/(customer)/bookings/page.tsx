...
import { BOOKING_STATUS_LABEL, type BookingStatusValue } from "@/src/features/booking/domain/value-objects/booking-status.vo";
    ...
    <div className="min-h-screen bg-[#F8FAFC] text-[#111827]">
      ...
      <main className="mx-auto flex w-full max-w-7xl flex-col gap-6 px-4 py-6 sm:px-6 sm:py-8 lg:px-8">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {[
            ["Pending", pendingCount, "Awaiting approval"],
            ["Payment due", paymentDueCount, "Approved quotes"],
            ["Confirmed", confirmedCount, "Paid deposits"],
            ["Completed", reviewCount, "Review history"],
          ].map(([label, value, caption]) => (
            <CustomerCard key={label as string} className="p-5">
              <p className="text-xs font-extrabold uppercase tracking-[0.12em] text-slate-400">
                {caption}
              </p>
              <p className="mt-2 text-3xl font-black text-slate-950">{value}</p>
              <p className="mt-1 text-sm font-bold text-[#2563EB]">{label}</p>
            </CustomerCard>
          ))}
        </div>
        ...
        {bookings.length === 0 ? (
          <CustomerEmptyState
            ...
            action={<CustomerLinkButton href="/venues">Browse Venues</CustomerLinkButton>}
          />
        ) : (
          <div className="grid gap-5">
            {bookings.map((booking) => {
              const venue = getVenue(booking);
              const venueImage = venue?.venue_images?.[0]?.storage_path ?? undefined;
                      ...
                      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                        ...
                        <div className="rounded-2xl border border-[#E5E7EB] bg-[#F9FAFB] px-4 py-3 lg:text-right">
                          ...
                          <p className="mt-1 text-lg font-black text-[#111827]">
                            {formatCurrency(booking.status === "approved" ? booking.deposit_amount : booking.total_amount)}
                          </p>
                        </div>
                      </div>
