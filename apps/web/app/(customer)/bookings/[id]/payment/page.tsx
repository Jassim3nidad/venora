...
import { ArrowLeft, FileText, CheckCircle2, Clock3, AlertTriangle, ShieldCheck, XCircle } from "lucide-react";
...
import type { InvoiceRow, ReceiptRow, RefundRow } from "@/src/features/payments/types/payment.types";
...
export default async function BookingPaymentPage({ params }: Props) {
  ...
  const [{ data: invoices }, { data: receipts }, { data: refunds }] = await Promise.all([
    ...
    supabase.from("receipts").select("*").eq("booking_id", id).order("created_at", { ascending: false }),
    supabase.from("refunds").select("*").eq("booking_id", id).order("created_at", { ascending: false }),
  ]);
  ...
  const depositPercentage = totalAmount > 0 ? Math.round((depositAmount / totalAmount) * 100) : null;
  ...
  const activePendingTransaction = transactions.find((t: any) => t.status === "pending");
  const failedTransaction = !activePendingTransaction && transactions.find((t: any) => t.status === "failed");
  const expiredTransaction = !activePendingTransaction && transactions.find((t: any) => t.status === "expired");
  
  let paymentUiState: "payable" | "pending_provider" | "paid" | "failed" | "expired" | "not_payable" = "not_payable";
  ...
  const isOverdue = booking.payment_due_at && new Date(booking.payment_due_at).getTime() < Date.now();

  const venueName = Array.isArray(booking.venues) ? booking.venues[0]?.name : booking.venues?.name;
    ...
    <div className="bg-[#F8FAFC] text-[#111827] min-h-screen pb-12">
      <div className="mx-auto flex w-full max-w-[1280px] flex-col gap-6 px-4 py-6 sm:px-6 sm:py-8 lg:px-8">
        ...
        <div className="mt-4 flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 text-sm font-bold">
          ...
          <div className={`flex items-center gap-2 ${isPaid ? "text-green-700" : paymentUiState === "pending_provider" ? "text-amber-700" : "text-[#2563EB]"}`}>
            {isPaid ? <CheckCircle2 className="h-5 w-5" /> : paymentUiState === "pending_provider" ? <Clock3 className="h-5 w-5" /> : <div className="flex h-5 w-5 items-center justify-center rounded-full bg-[#2563EB] text-white text-[10px]">2</div>}
            {paymentUiState === "pending_provider" ? "Payment processing" : isPaid ? "Deposit paid" : "Deposit payment"}
            {(!isPaid && paymentUiState !== "pending_provider") && <span className="ml-2 rounded bg-[#EFF6FF] px-2 py-0.5 text-xs text-[#1D4ED8]">Current</span>}
          </div>
          <div className="hidden sm:block text-slate-300">→</div>
          <div className={`flex items-center gap-2 ${isPaid ? "text-green-700" : "text-slate-400"}`}>
            {isPaid ? <CheckCircle2 className="h-5 w-5" /> : <div className="flex h-5 w-5 items-center justify-center rounded-full border-2 border-slate-200 text-[10px]">3</div>}
            Booking confirmed
          </div>
        </div>
          ...
          <div className="grid gap-8 rounded-3xl border border-[#E5E7EB] bg-white p-6 sm:p-8 shadow-sm">
            ...
            <section>
              ...
              <dl className="grid gap-3 text-sm">
                ...
                <div className="flex items-center justify-between">
                  <dt className="font-medium text-slate-600">Remaining balance</dt>
                  <dd className="font-bold text-slate-700 tabular-nums">{formatCurrency(remainingBalance)}</dd>
                </div>
              </dl>
            </section>
            ...
            <section className="grid gap-5">
              {booking.payment_due_at && !isPaid && (
                <div className="rounded-2xl border border-slate-100 bg-slate-50 p-4">
                  <p className="text-sm font-bold text-slate-900">Payment due</p>
                  <p className="text-base text-slate-700 mt-1">{formatDate(booking.payment_due_at, true)}</p>
                  <p className={`text-sm mt-1 font-semibold ${isOverdue ? "text-red-600" : "text-amber-600"}`}>
              ...
              {isPaid ? (
                <div className="rounded-2xl border border-green-200 bg-green-50 p-5">
                  <div className="flex items-center gap-3 text-green-800">
                    ...
                    <div>
                      ...
                      <p className="text-sm mt-1 text-green-700">Your reservation deposit has been received. View your booking for the latest confirmation details.</p>
                    </div>
                  </div>
                </div>
              ) : (
                <BookingPaymentClientUI 
                  ...
                  providerReference={activePendingTransaction?.provider_reference}
                  startedAt={activePendingTransaction?.created_at ? formatDate(activePendingTransaction.created_at, true) : undefined}
                />
              )}
              ...
              {!isPaid && paymentUiState !== "pending_provider" && (
                <div className="text-sm text-slate-600 mt-2">
                  <span className="font-bold text-slate-900">What happens next?</span> After your deposit is confirmed, your booking will be reserved and its status will change to Confirmed.
                </div>
              )}
            </section>
            ...
            {!isPaid && (
              <div className="flex items-center gap-3 pt-6 border-t border-slate-100 text-sm text-slate-600">
                ...
                <p>
                  <span className="font-bold text-slate-700">Secure payment through PayMongo.</span> You’ll be redirected to PayMongo to choose an available payment method. Venora does not store your card details.
                </p>
              </div>
            )}
          </div>
              ...
              {invoices && invoices.length > 0 ? (
                <div className="grid gap-3">
                  {invoices.map((inv: any) => (
                    <div key={inv.id} className="flex items-start justify-between gap-4 py-3 border-b border-slate-100 last:border-0 last:pb-0">
                      <div className="flex gap-3">
                        <FileText className="h-5 w-5 text-slate-400 shrink-0 mt-0.5" />
                        <div className="grid gap-1 text-sm">
                          <p className="font-bold text-slate-900">Invoice {inv.invoice_number || "INV"}</p>
                          <p className="text-slate-600">Reservation deposit</p>
                          <p className="text-slate-500">{formatCurrency(inv.amount_due)} · Due {formatDate(inv.due_date)}</p>
                          {inv.hosted_invoice_url && (
                            <a href={inv.hosted_invoice_url} target="_blank" rel="noreferrer" className="text-[#2563EB] font-bold hover:underline mt-1">
                              View invoice
                            </a>
                          )}
                        </div>
                ...
                <p className="text-sm text-slate-500">No billing documents are available yet.</p>
              )}
            </section>
                ...
                <div className="relative border-l border-slate-200 ml-3 pl-5 grid gap-6 py-2">
                  {activityItems.map((item, idx) => (
                    <div key={`${item.type}-${item.id}-${idx}`} className="relative">
                      <div className={`absolute -left-[25px] top-1 h-2.5 w-2.5 rounded-full border-2 border-white ring-1 ring-slate-200 ${item.status === 'paid' ? 'bg-green-500' : item.status === 'failed' ? 'bg-red-500' : 'bg-slate-400'}`} />
                      <p className="text-xs font-semibold text-slate-500">{formatDate(item.date, true)}</p>
                      ...
                      {item.type === "invoice" ? (
                        ...
                        <div className="mt-1 grid gap-1 text-sm">
                          <p className="font-bold text-slate-900">
                            {item.status === "paid" ? "Deposit payment confirmed" : item.status === "failed" ? "Payment attempt failed" : "Payment attempt started"}
                          </p>
                          <p className="text-slate-600">{formatCurrency(item.amount)} through {item.provider || "provider"}</p>
                          {item.status === "pending" && <p className="text-amber-600 font-medium text-xs mt-0.5">Status: Awaiting confirmation</p>}
                          {item.status === "paid" && item.reference && <p className="text-slate-500 text-xs mt-0.5">Reference: {item.reference}</p>}
                        </div>
                      )}
                    </div>
                  ))}
