...
import { getRevenueTrend, getTopPackages, lastNMonthsRange } from "@/features/analytics/application/queries";

export const metadata: Metadata = { title: "Dashboard | Venora" };
...
export default async function CustomerDashboardPage() {
  ...
  const [
    ...
    supabase
      ...
      .eq("customer_id", user.id),
    getRevenueTrend(supabase, { kind: "customer", customerId: user.id }, lastNMonthsRange(12)),
    getTopPackages(supabase, { kind: "customer", customerId: user.id }),
  ]);

  const bookingRows = (bookings ?? []) as { id: string; status: string; total_amount: number | null; event_date: string }[];
  ...
  const upcomingBookings = bookingRows.filter(
    (booking) => booking.event_date >= today && !CLOSED_BOOKING_STATUSES.includes(booking.status),
  ).length;
  ...
  return (
    <main className="mx-auto flex w-full max-w-6xl flex-col gap-6 px-4 py-6 sm:px-6 sm:py-8 lg:px-8">
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <KpiCard label="Total Bookings" value={String(totalBookings)} icon="event_available" highlight />
        <KpiCard label="Total Spent" value={formatPeso(totalSpent)} icon="payments" />
        <KpiCard label="Upcoming Bookings" value={String(upcomingBookings)} icon="calendar_month" />
        <KpiCard label="Favorites" value={String(favoritesCount ?? 0)} icon="favorite" />
      </div>

      <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_360px]">
        <Panel>
          <PanelHeader title="Spending Over Time" description="Your booking spend, month by month." />
          <RevenueTrendChart data={revenueTrend} format="currency" />
        </Panel>

        <Panel>
          <PanelHeader title="Booking Status" description="Where your bookings currently stand." />
          <StatusDistributionChart data={statusMix} />
        </Panel>
      </div>

      <Panel>
        <PanelHeader title="Your Top Venues" description="The venues and packages you book most often." />
