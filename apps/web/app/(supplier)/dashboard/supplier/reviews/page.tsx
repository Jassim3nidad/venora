...
export default async function SupplierReviewsPage() {
  ...
  const average = reviews.length
    ? reviews.reduce(
        (sum: number, row: any) => sum + Number(row.overall_rating),
        0,
      ) / reviews.length
    : 0;
  return (
    <DashboardSubPage
      title="Reviews"
      description="See verified customer feedback from completed supplier work."
    >
      ...
      {reviews.length === 0 ? (
        <EmptyState
          icon="rate_review"
          title="No supplier reviews yet"
          description="Verified customer reviews will appear after completed supplier jobs."
        />
      ) : (
