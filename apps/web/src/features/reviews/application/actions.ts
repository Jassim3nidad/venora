...
import {
  attachReviewPhotosSchema,
  deleteReviewPhotoSchema,
} from "../schemas/review-photo.schema";
...
function reviewErrorFromMessage(message: string) {
  ...
  if (
    ...
    normalized.includes("review_flags")
  ) {
...
async function getVenueSlugForReview(
  supabase: any,
  reviewId: string,
): Promise<string | null> {
...
export async function deleteReviewPhotoAction(rawInput: unknown) {
  return createServerAction(
    ...
    async (input) => {
      ...
      const review = Array.isArray(photo.reviews)
        ? photo.reviews[0]
        : photo.reviews;
      if (!review || review.customer_id !== userId) {
        throw new ForbiddenError(
          "You can only delete photos from your own review.",
        );
      }
