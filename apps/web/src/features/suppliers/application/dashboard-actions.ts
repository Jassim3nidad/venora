...
async function requireOwnedInquiry(supabase: any, supplierId: string, inquiryId: string) {
...
export async function upsertSupplierQuoteAction(rawInput: unknown) {
  return createServerAction(supplierQuoteSchema, async (input) => {
    const { supabase, supplier } = await requireSupplier();
    await requireOwnedInquiry(supabase, supplier.id, input.inquiryId);
    const { data, error } = await supabase.rpc("upsert_supplier_quote_dashboard", {
...
async function transitionQuote(rawInput: unknown, from: string, to: string) {
  return createServerAction(supplierQuoteIdSchema, async ({ quoteId }) => {
    const { supabase, supplier } = await requireSupplier();
    const payload = to === "sent" ? { status: to, sent_at: new Date().toISOString() } : { status: to };
    const { data, error } = await supabase
      ...
      .maybeSingle();
    if (error) throw new ValidationError(error.message);
    if (!data) throw new ValidationError("Quote is no longer eligible for this action.");
    revalidateSupplierDashboard();
    return { quoteId };
  }, rawInput);
}
...
export async function sendSupplierInquiryMessageAction(rawInput: unknown) {
  return createServerAction(supplierMessageSchema, async (input) => {
...
export async function setSupplierAvailabilityAction(rawInput: unknown) {
  return createServerAction(supplierAvailabilitySchema, async (input) => {
    const { supabase, user, supplier } = await requireSupplier();
    const { data: job } = await supabase
      ...
      .maybeSingle();
    if (job) throw new ValidationError("Confirmed jobs cannot be manually changed.");
    const { error } = await supabase.from("supplier_availability").upsert(
      { supplier_id: supplier.id, date: input.date, status: input.status, reason: input.reason ?? null, created_by: user.id },
...
export async function clearSupplierAvailabilityAction(rawInput: unknown) {
  return createServerAction(clearSupplierAvailabilitySchema, async (input) => {
    const { supabase, supplier } = await requireSupplier();
