...
export function SupplierProfileForm({
  ...
  const defaultValues: ProfileFormValues = {
    ...
    coverageRadiusKm: profile?.coverageRadiusKm ? String(profile.coverageRadiusKm) : "",
  ...
  const completionPercentage = Math.round((filledItems / completionItems.length) * 100);
  ...
  const getPriceUnitLabel = (unit: string) => {
    switch (unit) {
      ...
      case "per_pax": return "per pax";
      case "per_day": return "per day";
      default: return unit;
    }
  };

  const categoryName = categories.find(c => c.id === formValues.categoryId)?.name || "Category";
      ...
      <div className="mb-6 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div className="border-b border-slate-100 bg-slate-50 px-6 py-4">
          <div className="flex items-center justify-between">
            ...
            <span className="font-black text-[#2563EB]">{completionPercentage}%</span>
          </div>
        ...
        {missingItems.length > 0 && (
          <div className="px-6 py-4">
            <p className="text-sm font-semibold text-slate-600 mb-2">Complete your profile to improve visibility. Still needed:</p>
            <ul className="grid gap-1 sm:grid-cols-2 md:grid-cols-3">
              ...
              {missingItems.length > 6 && (
                <li className="text-xs text-slate-400 italic">...and {missingItems.length - 6} more</li>
              )}
            </ul>
          </div>
        )}
      ...
      <div className="flex flex-col xl:flex-row gap-8 items-start">
          ...
          <form id="profile-form" onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            ...
            >
              ...
              <label className="grid gap-1.5">
                <div className="flex justify-between items-end">
                  <span className="text-sm font-bold text-slate-700">Business Tagline</span>
                ...
                <input
                  ...
                  className="w-full h-11 rounded-lg border border-slate-200 bg-slate-50 px-3 text-sm font-semibold outline-none transition-all placeholder:text-slate-400 focus:border-[#2563EB] focus:bg-white focus:ring-4 focus:ring-[#2563EB]/10"
                />
                <p className="text-xs text-slate-500">A short sentence customers will see below your business name.</p>
                <FieldError message={errors.headline?.message} />
              </label>

              <label className="grid gap-1.5">
                <div className="flex justify-between items-end">
                  <span className="text-sm font-bold text-slate-700">Business Description</span>
                  <span className="text-xs text-slate-400 font-medium">
                ...
                />
                <p className="text-xs text-slate-500">Explain what you offer and what makes your business different.</p>
            ...
            >
              <div className="grid gap-6 sm:grid-cols-2">
                ...
                <label className="grid gap-1.5">
                  <span className="text-sm font-bold text-slate-700">Pricing Basis</span>
                  <select
                    {...register("priceUnit")}
                    className="w-full h-11 rounded-lg border border-slate-200 bg-slate-50 px-3 text-sm font-semibold outline-none transition-all focus:border-[#2563EB] focus:bg-white focus:ring-4 focus:ring-[#2563EB]/10"
                  >
                ...
                <label className="grid gap-1.5">
                  <span className="text-sm font-bold text-slate-700">Typical Response Time</span>
                  <div className="relative">
                    <input
                    ...
                    />
                    <span className="absolute right-3 top-1/2 -translate-y-1/2 text-sm font-medium text-slate-500">hours</span>
                  </div>
                </label>

                <label className="grid gap-1.5">
                  <span className="text-sm font-bold text-slate-700">Minimum Booking Notice</span>
                  <div className="relative">
                    <input
                    ...
                    />
                    <span className="absolute right-3 top-1/2 -translate-y-1/2 text-sm font-medium text-slate-500">days</span>
                  </div>
                </label>

                <label className="grid gap-1.5">
                  <span className="text-sm font-bold text-slate-700">Years in Business</span>
                  <div className="relative">
                    <input
                      ...
                      className="w-full h-11 rounded-lg border border-slate-200 bg-slate-50 px-3 pr-14 text-sm font-semibold outline-none transition-all focus:border-[#2563EB] focus:bg-white focus:ring-4 focus:ring-[#2563EB]/10"
                    />
                    <span className="absolute right-3 top-1/2 -translate-y-1/2 text-sm font-medium text-slate-500">years</span>
                  </div>
                </label>
                ...
                <label className="grid gap-1.5">
                  ...
                  <div className="relative">
                    <input
                      ...
                      className="w-full h-11 rounded-lg border border-slate-200 bg-slate-50 px-3 pr-16 text-sm font-semibold outline-none transition-all focus:border-[#2563EB] focus:bg-white focus:ring-4 focus:ring-[#2563EB]/10"
                    />
                    <span className="absolute right-3 top-1/2 -translate-y-1/2 text-sm font-medium text-slate-500">people</span>
                  </div>
                </label>
              </div>
            </SectionCard>
            ...
            >
              <div className="grid gap-6 sm:grid-cols-2">
                ...
                <label className="grid gap-1.5">
                  <span className="text-sm font-bold text-slate-700">Instagram URL</span>
                  <input
        ...
        <div className="hidden xl:block w-80 2xl:w-[360px] shrink-0">
          <div className="sticky top-6">
            <h3 className="text-sm font-bold text-slate-500 uppercase tracking-wider mb-3">Live Profile Preview</h3>
            <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm flex flex-col pointer-events-none">
              ...
              <div className="relative h-32 w-full bg-slate-100 border-b border-slate-100">
                {formValues.heroImageUrl ? (
                  <img src={formValues.heroImageUrl} alt="" className="h-full w-full object-cover" />
                ) : (
              ...
              <div className="relative p-5 pt-0 flex-1 flex flex-col">
                ...
                <div className="absolute -top-10 left-5 h-20 w-20 rounded-xl border-4 border-white bg-slate-100 shadow-sm overflow-hidden flex items-center justify-center">
                  {formValues.profileImageUrl ? (
                    <img src={formValues.profileImageUrl} alt="" className="h-full w-full object-cover" />
                  ) : (
                ...
                <div className="mt-5 space-y-2.5">
                  {formValues.basePrice && (
                    <div className="flex items-center gap-2 text-sm text-slate-600">
                      ...
                      <span className="font-semibold text-slate-900">Starts at ₱{formatCurrency(formValues.basePrice)}</span>
                      <span className="text-slate-500"> {getPriceUnitLabel(formValues.priceUnit)}</span>
                    </div>
                  )}
                  ...
                  {formValues.serviceAreasText && (
                    <div className="flex gap-2 text-sm text-slate-600">
                      ...
                      <span className="line-clamp-2">{formValues.serviceAreasText}</span>
                    </div>
                  )}
                </div>
                ...
                <div className="mt-5 border-t border-slate-100 pt-4 flex flex-wrap gap-x-4 gap-y-2 text-xs text-slate-500">
                  {formValues.yearsInBusiness && (
                    <div className="flex items-center gap-1.5"><Clock className="h-3.5 w-3.5"/> {formValues.yearsInBusiness} yrs exp</div>
                  )}
                  {formValues.teamSize && (
                    <div className="flex items-center gap-1.5"><Users className="h-3.5 w-3.5"/> Team of {formValues.teamSize}</div>
                  )}
                </div>
              </div>
