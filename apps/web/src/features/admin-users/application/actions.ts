...
const NEW_STATUS_FOR_ACTION = { suspend: "suspended", reactivate: "active" } as const;

export async function setAccountStatusAction(rawInput: unknown) {
  return createServerAction(setAccountStatusSchema, async (input) => {
    ...
    await requirePermission(input.action === "suspend" ? "users.suspend" : "users.reactivate");
