...
const MAX_GUEST_CSV_BYTES = 1_000_000;
...
export function parseGuestCsv(csv: string): GuestImportInput[] {
  if (new TextEncoder().encode(csv).byteLength > MAX_GUEST_CSV_BYTES) {
  ...
  if (rows.length - 1 > MAX_GUEST_IMPORT_ROWS) {
    throw new Error(
      `CSV is limited to ${MAX_GUEST_IMPORT_ROWS} guests at a time.`,
    );
  }
