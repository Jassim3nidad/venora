...
import { hasAllowedFileSignature } from "@/src/lib/security/file-signatures";
...
export async function generateVerificationUploadUrlsAction(
  ...
  try {
    ...
    if (
      existingApp &&
      ["pending", "approved", "denied"].includes(existingApp.status)
    ) {
      return {
        success: false,
        error: "Cannot upload documents for a submitted application.",
      };
    }
    ...
    for (const file of files) {
      if (file.size <= 0 || file.size > MAX_SIZE) {
        return {
          success: false,
          error: `Invalid file size for ${file.name}. Max 20MB.`,
        };
      }
      ...
      if (!allowedExts) {
        return {
          success: false,
          error: `Disallowed MIME type for ${file.name}.`,
        };
      }
      ...
      if (!ext || !allowedExts.includes(ext)) {
        return {
          success: false,
          error: `Extension mismatch for ${file.name}.`,
        };
      }
  ...
  try {
    ...
    for (const path of paths) {
      ...
      if (!mime || !Object.keys(ALLOWED_MIME_TYPES).includes(mime)) {
        ...
        return {
          success: false,
          error: "Uploaded file has disallowed MIME type. Removed.",
        };
      }

      const { data: uploadedBlob, error: downloadError } =
        await supabase.storage.from("verification-docs").download(path);

      if (downloadError || !uploadedBlob) {
        return {
          success: false,
          error: "Could not inspect uploaded file content.",
        };
      }

      const signatureBytes = new Uint8Array(
        await uploadedBlob.slice(0, 8).arrayBuffer(),
      );

      if (!hasAllowedFileSignature(signatureBytes, mime)) {
        await supabase.storage.from("verification-docs").remove([path]);
        return {
          success: false,
          error: "Uploaded file content does not match its MIME type. Removed.",
        };
      }
    }
