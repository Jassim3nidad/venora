...
export async function getSystemSettings(): Promise<{
  ...
  const { data, error } = await supabase
    ...
    .select(
      "key, category, value, description, is_dangerous, updated_at, profiles:updated_by (full_name)",
    );
...
export async function getSettingHistory(key: string): Promise<{
  ...
  const { data, error } = await supabase
    ...
    .select(
      "id, setting_key, previous_value, new_value, reason, created_at, profiles:actor_id (full_name)",
    )
    .eq("setting_key", key)
