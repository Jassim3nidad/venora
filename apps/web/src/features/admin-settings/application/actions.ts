...
export async function updateSystemSettingAction(rawInput: unknown) {
  return createServerAction(
    ...
    async (input) => {
      ...
      if (!definition)
        throw new ValidationError(`Unknown setting: ${input.key}`);

      const actualType = Array.isArray(input.value)
        ? "string[]"
        : typeof input.value;
      const expectedType =
        definition.valueType === "string[]" ? "string[]" : definition.valueType;
      if (actualType !== expectedType) {
        throw new ValidationError(
          `"${definition.label}" expects a ${definition.valueType} value`,
        );
      }

      if (definition.isDangerous && !input.reason?.trim()) {
        throw new ValidationError(
          `A reason is required to change "${definition.label}"`,
        );
      }
