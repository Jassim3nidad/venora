...
function bookingErrorFromMessage(message: string) {
  ...
  if (normalized.includes("permission") || normalized.includes("only the booking customer")) {
  ...
  if (normalized.includes("duplicate key") || normalized.includes("already reviewed")) {
...
export async function createBookingAction(rawInput: unknown) {
  return createServerAction(createBookingSchema, async (input) => {
    ...
    if (!user) {
      throw new UnauthorizedError("You must be signed in to submit a booking inquiry.");
    }
    ...
    if (isMissingRpcError(error)) {
      ...
      if (input.guestCount < capacityMin || input.guestCount > capacityMax) {
        throw new ValidationError("Guest count is outside this venue capacity.");
      }
      ...
      if (input.packageId) {
        ...
        if (
          ...
          throw new ValidationError("Guest count is outside the selected package limits.");
        }
      }
...
export async function cancelBookingAction(rawInput: unknown) {
  return createServerAction(cancelBookingSchema, async (input) => {
    const supabase = (await createClient()) as any;
...
export async function startBookingPaymentAction(rawInput: unknown) {
  return createServerAction(startBookingPaymentSchema, async (input) => {
    const appUrl =
...
export async function completeBookingAction(rawInput: unknown) {
  return createServerAction(completeBookingSchema, async (input) => {
    const supabase = (await createClient()) as any;
...
export async function submitBookingReviewAction(rawInput: unknown) {
  return createServerAction(bookingReviewSchema, async (input) => {
    const supabase = (await createClient()) as any;
    const {
      data: { user },
    } = await supabase.auth.getUser();
    ...
    if (!user) {
      throw new UnauthorizedError("You must be signed in to review a booking.");
    }
