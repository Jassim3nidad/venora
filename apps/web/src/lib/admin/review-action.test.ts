...
import { createReviewActionSchema, throwIfReviewActionError } from "./review-action";
...
describe("throwIfReviewActionError", () => {
  ...
  it("maps permission-check failures to ForbiddenError", () => {
    expect(() => throwIfReviewActionError({ message: "You do not have permission to perform this venue review action" }))
  ...
  it("maps 'not found' failures to NotFoundError without duplicating the phrase", () => {
    try {
      ...
      expect((error as NotFoundError).message).not.toContain("not found not found");
    }
  });

  it("maps every other Postgres RAISE EXCEPTION message to ValidationError", () => {
    expect(() => throwIfReviewActionError({ message: "A reason is required to reject a venue" }))
...
describe("createReviewActionSchema", () => {
  ...
  it("accepts a valid action with a uuid id", () => {
    const result = schema.safeParse({ id: "11111111-1111-1111-1111-111111111111", action: "approve" });
    expect(result.success).toBe(true);
  });

  it("rejects an action outside the allowed list", () => {
    const result = schema.safeParse({ id: "11111111-1111-1111-1111-111111111111", action: "delete_everything" });
