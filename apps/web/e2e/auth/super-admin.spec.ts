...
test.describe("Super administrator", () => {
  ...
  test("overview quick-link cards show every module (super_admin holds all permissions)", async ({
    page,
  }) => {
  ...
  test("AI configuration page never renders a provider API key", async ({
    page,
  }) => {
  ...
  test("cannot demote its own super_admin tier (server-side guard rejects it)", async ({
    page,
  }) => {
  ...
  test("cannot retrieve full stored secrets through any admin endpoint", async ({
    page,
  }) => {
