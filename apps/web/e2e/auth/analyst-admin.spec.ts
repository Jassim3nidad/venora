...
test.describe("Analyst administrator — allowed", () => {
  ...
  test("navigation shows only permitted modules (Overview, Reports, Audit Logs)", async ({
    page,
  }) => {
    await page.goto("/admin");
  ...
  test("overview quick-link cards match analyst permissions", async ({
    page,
  }) => {
    await page.goto("/admin");
    ...
    await expect(
      page.getByRole("link", { name: "Review Applications" }),
    ).toHaveCount(0);
    await expect(page.getByRole("link", { name: "Review Venues" })).toHaveCount(
      0,
    );
    await expect(
      page.getByRole("link", { name: "Manage Reviews" }),
    ).toHaveCount(0);
  });
});
...
test.describe("Analyst administrator — forbidden mutations and modules", () => {
  ...
  for (const route of forbiddenReadRoutes) {
    test(`cannot access ${route} (no permission for that module at all)`, async ({
      page,
    }) => {
  ...
  const forbiddenMutations: Array<[string, string, Record<string, unknown>]> = [
    ...
    [
      "approve a venue",
      "/api/admin/venues/approve",
      { venueId: "d131d99a-5300-4de4-a23f-03abf6c61c1d" },
    ],
    [
      "reject a venue",
      "/api/admin/venues/reject",
      { venueId: "d131d99a-5300-4de4-a23f-03abf6c61c1d" },
    ],
    [
      "suspend a venue",
      "/api/admin/venues/suspend",
      { venueId: "d131d99a-5300-4de4-a23f-03abf6c61c1d" },
    ],
    [
      "approve a supplier",
      "/api/admin/suppliers/approve",
      { supplierId: "00000000-0000-0000-0000-000000000000" },
    ],
    [
      "reject a supplier",
      "/api/admin/suppliers/reject",
      { supplierId: "00000000-0000-0000-0000-000000000000" },
    ],
    [
      "suspend a supplier",
      "/api/admin/suppliers/suspend",
      { supplierId: "00000000-0000-0000-0000-000000000000" },
    ],
    [
      "create a commission rule",
      "/api/admin/commissions",
      { scope: "global", percentage: 5 },
    ],
    [
    ...
    [
      ...
      "/api/admin/commissions/override",
  ...
  test("cannot escalate its own tier or assign itself another role", async ({
    page,
  }) => {
    const res = await page.request.post(
      ...
      {
        data: {
          targetUserId: "495ead5d-2ad1-424d-bbbc-b8469f9e5c57",
          newTier: "super_admin",
        },
        failOnStatusCode: false,
      },
  ...
  test("cannot retrieve full AI provider keys through any endpoint", async ({
    page,
  }) => {
    const res = await page.request.get("/api/admin/ai-configuration/secrets", {
      failOnStatusCode: false,
    });
    expect([401, 403, 404, 405]).toContain(res.status());
  });
  ...
  test("cannot mutate append-only audit logs", async ({ page }) => {
    const putRes = await page.request.put("/api/admin/audit-logs/1", {
      data: {},
      failOnStatusCode: false,
    });
    expect([401, 403, 404, 405]).toContain(putRes.status());
    const delRes = await page.request.delete("/api/admin/audit-logs/1", {
      failOnStatusCode: false,
    });
...
test.describe("Analyst administrator — responsive", () => {
  for (const [name, viewport] of Object.entries(VIEWPORTS)) {
    test(`loads /admin cleanly at ${name} (${viewport.width}x${viewport.height})`, async ({
      page,
    }) => {
