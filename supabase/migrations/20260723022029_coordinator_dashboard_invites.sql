-- Add policy to allow authenticated users to see their own pending invitations by email
DROP POLICY IF EXISTS "org_member_invites.select.pending_by_email" ON public.organization_member_invitations;
CREATE POLICY "org_member_invites.select.pending_by_email"
  ON public.organization_member_invitations FOR SELECT
  TO authenticated
  USING (
    status = 'pending'
    AND email = (SELECT lower(auth_users.email) FROM auth.users auth_users WHERE auth_users.id = auth.uid())
  );

-- Function to respond to an invitation by ID (for dashboard acceptance/rejection)
...
REVOKE ALL ON FUNCTION public.respond_to_organization_member_invitation_by_id(uuid, boolean) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.respond_to_organization_member_invitation_by_id(uuid, boolean) TO authenticated;
